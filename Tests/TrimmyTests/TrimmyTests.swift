...
struct TrimmyTests {
    ...
    func `preserves percent encoding in kept params`() {
        let cleaner = TextCleaner()
        // %3A is a colon; percentEncodedQueryItems preserves it rather than decoding and re-encoding
    ...
    // MARK: - Built-in domain rules (parsed from defaultRulesText, as the app does at runtime)
